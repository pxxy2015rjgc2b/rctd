	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			for (var len = 0; len < Handle.lists.length; len++) {
				str += '<tr >';
				str += '<td id="rctd_list_id"   style="display:none;">' + Handle.lists[len].rctd_list_id + '</td>';
...
//				str += '<td>' + Handle.lists[len].list_community + '</td>';
				...
				str += '<td><span id="'+Handle.lists[len].rctd_list_id+ '" onclick="editCount(this)"><i class="fa fa-edit"></i></span></td>';
				str += '<td><input class="checkbox" name="checkbox_select" type="checkbox"/></td>';
				str += '</tr>';
			}
			/*//修改信息入口
			$('i[action="LinkToModifySnece"]').click(function() {
				window.location.href = "/rctd/manager/getListById?list_id=" + $(this).parents('tr').childNode[0].attr('rctd_list_id').innerHTML;
			});*/
			//console.log(str);
			$('#list tbody').html(str);
...
var skipToArbitrarilyPage = function() {
	if ($('#skipPage').val() >= 1 && $('#skipPage').val() <= queryConditionTemp.totalPage) {
