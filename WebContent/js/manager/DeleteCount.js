				...
				buttons : {
					删除 : {
						...
						action : function() {
							...
							/*
							 * //console.log("dd"); var check_arr=new Array();
							 * var checkbox_select =
							 * document.getElementsByName("checkbox_select");
							 * for(var i=0;i<checkbox_select.length;i++){
							 * if(checkbox_select[i].checked){ var
							 * checked=checkbox_select[i].parentNode.parentNode.children[0].innerHTML;
							 * check_arr.push(checked); //console.log(checked); } }
							 */
							var checkbox_select = document
									.getElementsByName("checkbox_select");
							// var rctd_list_id =
							// document.getElementsById("rctd_list_id");
							...
							xhr
									.open("POST",
											"/rctd/manager/manager_deleteList");
							xhr.send(formData);
						}
					},
					取消 : function() {
